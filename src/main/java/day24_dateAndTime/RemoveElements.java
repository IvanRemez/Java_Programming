...
public class RemoveElements {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (list.get(i) < 4){
                list.remove(i); // *do NOT use remove() in Loop -> removeIf() used instead
            }
        }
        System.out.println(list);
        System.out.println("-------------------------------------------------------");
        ...
        list2.removeIf( p -> p < 4);
                    // ^^ predicate (used as temp variableName for the elements)
        ...
        names.removeIf(each -> each.startsWith("J")); // removes each element that starts with "J"
